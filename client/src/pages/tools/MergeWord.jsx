...
export default function MergeWord({ navTo }) {
  ...
  const handleSubmit = async () => {
    if (files.length < 2) {
      setError('Please select at least 2 .docx files to merge.');
      return;
    }
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="tool-page">
      ...
      <div className="tool-header">
        <h1 className="tool-title">Merge Word Documents</h1>
        <p className="tool-subtitle">Combine multiple .docx files into a single document.</p>
      </div>
        ...
        {!result ? (
          <>
            ...
            >
              ...
              <div className="dropzone-text">Click or drag Word files here</div>
              <div className="dropzone-sub">Supports multiple .docx files</div>
              <input 
                id="fileInput"
                type="file" 
                multiple 
            ...
            >
              {loading ? <div className="spinner" style={{ width: 18, height: 18, margin: 0 }} /> : 'Merge Word Files'}
            </button>
          </>
        ) : (
